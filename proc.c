...
static void procDumpP3P4(struct proc *p, char *state, int elapsedTime);
...
userinit(void)
{
  ...
  //Initialize processes before initializing the free list
  acquire(&ptable.lock);
  ...
  //State Transition EMBRYO -> RUNNABLE
  if (stateListRemove(&ptable.pLists.embryo, &ptable.pLists.embryoTail, p) == -1)
  ...
  //Since this is the first proc, put it on the ready list
...
exit(void)
{
  ...
  ///// State Transition RUNNING -> ZOMBIE
...
wait(void)
{
  ...
  for(;;){
    ...
    for(p = ptable.pLists.zombie; p; p = p->next) {
      ...
      ///// State Transition ZOMBIE -> UNUSED /////
...
scheduler(void)
{
  ...
  for(;;){
    ...
    for (p = ptable.pLists.ready; p; p = p->next) {
      ...
      ///// State Transition RUNNABLE -> RUNNING /////
